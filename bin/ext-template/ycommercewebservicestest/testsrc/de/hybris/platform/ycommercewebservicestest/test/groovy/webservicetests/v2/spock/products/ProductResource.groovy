...
@ManualTest
@Unroll
class ProductResource extends AbstractSpockFlowTest {
	...
	def "Search for product: #format"(){
		...
		with(response) {
			...
			data.sorts.size() > 0
			data.pagination
			'20'.equals(data.pagination.pageSize.toString())
			'2'.equals(data.pagination.totalPages.toString())
			'0'.equals(data.pagination.currentPage.toString())
			'28'.equals(data.pagination.totalResults.toString())
		}
		where:
		format << [XML, JSON]
	}
