...
class CartTests extends BaseWSTest {
	...
	void testEnableOrderPromotionByCustomer() {
		...
		def customerTests = new CustomerTests()
		def uid = customerTests.registerUser()
		def access_token = testUtil.getAccessToken(uid, PASSWORD, testUtil.TRUSTED_CLIENT_ID, testUtil.TRUSTED_CLIENT_SECRET)
	...
	void testEnableOrderPromotionByCustomerWithNonTrustedClientShouldFail() {
		...
		def customerTests = new CustomerTests()
		def uid = customerTests.registerUser()
		def access_token = testUtil.getAccessToken(uid, PASSWORD, testUtil.CLIENT_ID, testUtil.CLIENT_SECRET)
	...
	void testCreateFirstPayment() {
		...
		assert response.paymentInfo.defaultPaymentInfo == true // first payment is always default
	}
	...
	void testCreateDefaultPaymentWithoutSavingIt() {
		...
		assert response.paymentInfo.defaultPaymentInfo == false // payment cannot be default if it's not saved
	}
	...
	void setStockStatus(String status, String productId) {
		String host = testUtil.HOST;
		def con = testUtil.getSecureConnection("https://${host}:9002/ws410/rest/catalogs/wsTestProductCatalog/catalogversions/Online/products/${productId}", 'GET', 'XML', HttpURLConnection.HTTP_OK, null, null, "admin:nimda")
