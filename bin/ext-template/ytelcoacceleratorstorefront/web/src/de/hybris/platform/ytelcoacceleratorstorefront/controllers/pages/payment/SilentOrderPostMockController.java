...
public class SilentOrderPostMockController extends AbstractController
{
	...
	public String handleFormPost(@Valid final SopPaymentDetailsForm form, final BindingResult bindingResult,
            @RequestParam("targetArea") final String targetArea,
			...
			final RedirectAttributes redirectAttributes)
	{
		...
		if (bindingResult.hasErrors())
		{
            ...
            redirectAttributes.addFlashAttribute("org.springframework.validation.BindingResult.sopPaymentDetailsForm", bindingResult);
		...
		else
		{
			...
			try
			{
				...
				else
				{
					...
					// enrich result data with form data, which is not provided from the facade call
					if (BooleanUtils.isNotTrue(form.getNewBillingAddress()) && null != getCheckoutFacade().getCheckoutCart()
					...
					{
						newPaymentSubscription.getBillingAddress().setTitleCode(form.getBillingAddress().getTitleCode());
					}
