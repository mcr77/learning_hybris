...
/**
 * Test class for {@DefaultChangeSetBuilder}
 */
@IntegrationTest
public class DefaultChangeSetBuilderTest extends ExtendedServicelayerTransactionalBaseTest
{
	...
	{
		// given
		final Collection<ModelWrapper> wrappers = prepareWrappers(Arrays.<AbstractItemModel> asList(unit1), PersistenceOperation.SAVE);
	...
	{
		// given
		final Collection<ModelWrapper> wrappers = prepareWrappers(Arrays.<AbstractItemModel> asList(unit3), PersistenceOperation.SAVE);
	...
	public void testDeleteSingleUnit()
	{
		...
		final Collection<ModelWrapper> wrappers = prepareWrappers(Arrays.<AbstractItemModel> asList(unit1), PersistenceOperation.DELETE);
	...
	public void testBuildForCustomerWithAddress()
	{
		...
		assertThat(relationChangeSet.getInsertManyToManyRelationRecords().iterator().next()).isInstanceOf(
				InsertManyToManyRelationRecord.class);
	...
	public void testBuildForCustomerRemoveAddresses()
	{
		...
		assertThat((Collection) Iterables.get(changes, 0).getValue()).isEmpty();
	}
	...
	public void createCategoryWithKeywords()
	{
		...
		final Collection<InsertManyToManyRelationRecord> insertRecords = locRelationChanges.getRelationChangeForLanguage(
				Locale.ENGLISH).getInsertManyToManyRelationRecords();
	...
	public void removeKeywordsFromCategory()
	{
		...
		final Collection<ModelWrapper> wrappers = prepareWrappers(Arrays.<AbstractItemModel> asList(cat), PersistenceOperation.SAVE);
		...
		assertThat(relationChanges.getRemoveManyToManyRelationsRecords().iterator().next()).isInstanceOf(
				RemoveManyToManyRelationsRecord.class);
	...
	public void removeCategoryFromKeyword()
	{
		...
		final LocalizedRelationChanges locRelationChanges = (LocalizedRelationChanges) changeSet.getRelationChanges().get(
				CATEGORY_TO_KEYWORD_RELATION);
		assertThat(locRelationChanges.getRelationChangeForLanguage(Locale.ENGLISH)).isNotNull();
