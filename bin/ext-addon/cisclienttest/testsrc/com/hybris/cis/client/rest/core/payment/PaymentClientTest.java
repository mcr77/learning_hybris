...
public class PaymentClientTest extends ServicelayerTest
{
	...
	{
		final RestResponse<Void> responseHead = paymentClient.pspUrl(CLIENT_REF);
		final URI location = responseHead.getLocation();
		Assert.assertTrue(location.toString().matches(".*acceleratorservices/sop-mock/process"));
	}

	@Test
	public void shouldAcceptExternalAuthorization()
	{
		final RestResponse<CisPaymentTransactionResult> response = paymentClient.handleExternalAuthorization(CLIENT_REF,
	...
	public void shouldAcceptProfileAuthorization() throws URISyntaxException
	{
		...
		final RestResponse<CisPaymentTransactionResult> response = paymentClient.authorizeWithProfile(CLIENT_REF,
	...
	public void shouldRejectProfileAuthorization() throws URISyntaxException
	{
		...
		final RestResponse<CisPaymentProfileResult> response = paymentClient.addCustomerProfile(CLIENT_REF, "asdf", errReq);
	...
	public void shouldCaptureTest() throws URISyntaxException
	{
		...
		RestResponse<CisPaymentTransactionResult> response = this.paymentClient.authorizeWithProfile(CLIENT_REF,
				cisReferencePaymentRequest, new URI(getPaymentBaseUrl() + "/profiles/9997000011397391/"));
		...
		response = paymentClient.capture(CLIENT_REF, response.getLocation(), paymentRequest);
