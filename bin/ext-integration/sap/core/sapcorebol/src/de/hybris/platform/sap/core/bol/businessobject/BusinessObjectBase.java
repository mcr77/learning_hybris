...
public class BusinessObjectBase implements BusinessObject, BackendAware, Cloneable
{
	 ...
	 * 
	 * @param initialize
	 *           if true, backend object is reset first
	 */
	...
	protected void determineBackendObject(final boolean initialize)
	{
		...
		if (backendObject == null)
		{
			...
			{
				final String backendType = getBackendType();
				if (backendType == null || backendType.isEmpty())
				{
					throw new CoreBaseRuntimeException("No backend type available for Backend Object determination!");
				}
				final Class<?> backendInterfaceName = this.getClass().getAnnotation(BackendInterface.class).value();
				final String[] backendBeans = genericFactory.getBeanNamesForType(backendInterfaceName);
				for (int i = 0; i < backendBeans.length; i++)
				{
					if (genericFactory.findAnnotationOnBean(backendBeans[i], BackendType.class) != null
							&& !genericFactory.findAnnotationOnBean(backendBeans[i], BackendType.class).value().isEmpty()
							&& genericFactory.findAnnotationOnBean(backendBeans[i], BackendType.class).value().equals(backendType))
					{
						if (initialize)
						{
							genericFactory.removeBean(backendBeans[i]);
						}
						backendObject = (BackendBusinessObject) genericFactory.getBean(backendBeans[i]);
						break;
					}
				}
				if (backendObject == null)
				{
					throw new CoreBaseRuntimeException("No backend object of type " + backendInterfaceName.getName()
							+ " has the @BackendType annotation implemented");
				}
			}
			else
			{
				throw new CoreBaseRuntimeException("The business object has no @BackendInterface annotation");
			}
		}
	}
