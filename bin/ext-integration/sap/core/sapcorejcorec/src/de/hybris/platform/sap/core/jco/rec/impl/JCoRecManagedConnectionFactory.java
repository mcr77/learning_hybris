...
public class JCoRecManagedConnectionFactory implements JCoManagedConnectionFactory
{
	 ...
	 */
	private JCoRecRepository recordingRepository;
	...
	public JCoConnection getManagedConnection(final String connectionName, final String callerId)
	{
		...
		{
			case RECORDING:
				return new JCoRecConnection(wrappedFactory.getManagedConnection(connectionName, this.getClass().getName()),
						recordingRepository, recordingMode);
			case PLAYBACK:
				return new JCoRecConnection(null, recordingRepository, recordingMode);
			case OFF:
				return wrappedFactory.getManagedConnection(connectionName, this.getClass().getName());
			default:
	...
	public void startRecorder(final JCoRecMode recordingMode, final String directoryPath, final String key)
	{
		...
		if (this.recordingMode == JCoRecMode.RECORDING)
		{
			...
			recordingRepository = new JCoRecRepository(key, JCoRecMode.RECORDING);
			return;
		}

		if (this.recordingMode == JCoRecMode.PLAYBACK)
		{
			recordingRepository = new JCoRecRepository(key, JCoRecMode.PLAYBACK);
			recordingRepository.parseRepositoryFile(repositoryFile);
