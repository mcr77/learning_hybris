...
public class DefaultSAPB2BUnitService extends DefaultB2BUnitService {
	...
	private static final Logger LOGGER = Logger
			.getLogger(DefaultSAPB2BUnitService.class);

	/*
	 * Gets recursively all members of type B2BUnitModel for given B2BUnit and
	 * all their sub B2BUnits; is the given B2BUnit a sales area dependent
	 * B2BUnit this method gets all members of type B2BUnit for current
	 * B2BUnit's root B2BUnit and their sub B2BUnit
	 * 
	 * @see
	 * de.hybris.platform.b2b.services.impl.DefaultB2BUnitService#getBranch(
	...
	public Set<B2BUnitModel> getBranch(final B2BUnitModel unit) {
		...
		if (isSalesAreaDependentB2BUnit(unit)) {
			// determine branch (set of B2BUnit members) of sales area dependent
			// B2BUnit's root B2BUnit
	 ...
	 * 
	 * @param unit
	 * @return true = B2BUnit is a sales area dependent B2BUnit; false B2BUnit
	 *         is a sales area independent B2BUnit
	 */
	protected boolean isSalesAreaDependentB2BUnit(final B2BUnitModel unit) {
		if (unit != null) {
			...
			if (!salesAreaSuffix.isEmpty()
					&& unit.getUid().endsWith(salesAreaSuffix)) {
				return true;
			}
		}
	...
	/*
	 * Gets the parent B2BUnit of B2BCustomer; if the base store has assigned a
	 * SAP base store configuration with a sales area definition and the parent
	 * B2BUnit has a sub B2BUnit for this sales area as member the sales area
	 * dependent sub B2BUnit will returned
	 * 
	 * @see
	 * de.hybris.platform.b2b.services.impl.DefaultB2BUnitService#getParent(
	...
	public B2BUnitModel getParent(final B2BCustomerModel b2bCustomer) {
		...
		if (parentB2BUnit != null) {
			...
			if (salesAreaSuffix.isEmpty()) {
				...
				// B2BUnit
				final Set<PrincipalModel> parentB2BUnitMembers = parentB2BUnit
						.getMembers();
				if (parentB2BUnitMembers != null) {
					final String salesAreaDependentB2BUnitUID = parentB2BUnitUID
							+ salesAreaSuffix;
					for (final PrincipalModel parentB2BUnitMember : parentB2BUnitMembers) {
						...
						if (parentB2BUnitMember instanceof B2BUnitModel
								&& parentB2BUnitMember.getUid()
										.equalsIgnoreCase(
												salesAreaDependentB2BUnitUID)) {
							if (LOGGER.isDebugEnabled()) {
								LOGGER.debug("Returns sales area dependent B2B Unit with UID: ["
										+ parentB2BUnitMember.getUid() + "]");
							}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("No sales area dependent B2B Unit with UID: ["
							+ parentB2BUnitUID
							+ salesAreaSuffix
							+ "] found! Returns sales area independent B2B Unit with UID: ["
							+ parentB2BUnitUID + "]");
				}
			}
	...
	protected String getSalesAreaSuffix() {
		...
		if (currentBaseStore != null
				&& currentBaseStore.getSAPConfiguration() != null) {
			// SALES ORG
			final String salesOrganization = currentBaseStore
					.getSAPConfiguration().getSapcommon_salesOrganization();
			// DISTRIBUTION CHANNEL
			String distributionChannel = currentBaseStore.getSAPConfiguration()
					.getSapcommon_distributionChannel();
			// DIVISION
			String division = currentBaseStore.getSAPConfiguration()
					.getSapcommon_division();
			// check whether no part of sales area is blank
			if (!StringUtils.isBlank(salesOrganization)
					&& !StringUtils.isBlank(distributionChannel)
					&& !StringUtils.isBlank(division)) {
				// retrieve common distribution channel
				distributionChannel = getCommonDistributionChannel(
				...
				salesAreaSuffix = "_" + salesOrganization + "_"
						+ distributionChannel + "_" + division;
			}
		}
	 ...
	 */
	protected String getCommonDistributionChannel(
	 ...
	 */
	protected String getCommonDivision(final String salesOrganization,
