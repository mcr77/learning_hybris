...
public class DefaultSAPB2BDeliveryAddressesLookupStrategy extends DefaultB2BDeliveryAddressesLookupStrategy
{
			...
			final boolean visibleAddressesOnly)
	{
		...
		else
		{
			if (deliveryAddressesForB2BUnit != null && !deliveryAddressesForB2BUnit.isEmpty())
			{
				return filterDeliveryAddressesWithoutSapCustomerID(deliveryAddressesForB2BUnit);
		...
		return filterDeliveryAddressesWithoutSapCustomerID(deliveryAddresses);
	}

	protected List<AddressModel> filterDeliveryAddressesWithoutSapCustomerID(final List<AddressModel> deliveryAddresses)
	{

		final List<AddressModel> deliveryAddressesWithSapCustomerID = new ArrayList<AddressModel>();
		for (final AddressModel deliveryAddress : deliveryAddresses)
		{
			if (deliveryAddress.getSapCustomerID() != null && !deliveryAddress.getSapCustomerID().isEmpty())
			{
				deliveryAddressesWithSapCustomerID.add(deliveryAddress);
			}
		}
		return deliveryAddressesWithSapCustomerID;
