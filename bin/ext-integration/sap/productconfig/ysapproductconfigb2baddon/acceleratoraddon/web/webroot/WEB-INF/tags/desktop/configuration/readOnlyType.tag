...
<div id="${cstic.key}.readOnly" class="${cssConf:valueStyleClass(cstic)}">
	<c:choose>
		<c:when test="${fn:length(cstic.domainvalues) gt 0}">
			<c:forEach var="value" items="${cstic.domainvalues}">
				<c:if test="${value.selected}">
					<div class="product-config-csticValueLabelWithoutSelect product-config-csticValueLabelWithoutSelect">
		...
		<c:otherwise>
			<div class="product-config-csticValueLabel product-config-csticValueLabelWithoutSelect">
