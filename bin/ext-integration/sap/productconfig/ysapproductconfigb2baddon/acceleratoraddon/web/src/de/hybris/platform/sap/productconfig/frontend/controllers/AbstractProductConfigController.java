...
public abstract class AbstractProductConfigController extends AbstractPageController
{
			...
			final BindingResult bindingResult, final UiGroupData group)
	{
		...
		{
			final boolean restoreValidationError = latestCstic.isVisible() && latestCstic.getType() != UiType.READ_ONLY
	...
	{
		final String cartItemKey = getSessionService().getAttribute(
				PRODUCT_CART_ITEM_SESSION_PREFIX + configData.getKbKey().getProductCode());
		if (cartItemKey != null)
		{
	...
	private ConfigurationData loadNewConfiguration(final KBKeyData kbKey, final String selectedGroup)
	{
		final ConfigurationData configData;
	...
	private ConfigurationData reloadConfiguration(final KBKeyData kbKey, final String configId, final String selectedGroup,
