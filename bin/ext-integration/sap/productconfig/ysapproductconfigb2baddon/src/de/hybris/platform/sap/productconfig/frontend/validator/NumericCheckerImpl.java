...
public class NumericCheckerImpl implements NumericChecker
{
	...
	private static final String NOT_NUMERIC_ERROR = "sapproductconfig.value.not.numeric";
	private static final String TOO_LONG_FRACTION = "sapproductconfig.value.too.long.fraction";
	private static final String TOO_MANY_DIGITS = "sapproductconfig.value.too.many.digits";
	private static final String NO_NEGATIVE_NUMBER = "sapproductconfig.value.no.negative";
	...
	public void validate(final CsticData cstic, final Errors errorObj)
	{
		...
		catch (final ParseException e)
		{
			final Object[] args =
			{ value };
			errorObj.rejectValue("value", NOT_NUMERIC_ERROR, args, "Not a number");
		...
		{
			final Object[] args = {};
			errorObj.rejectValue("value", NO_NEGATIVE_NUMBER, args, "Not a number");
			cstic.setCsticStatus(CsticStatusType.ERROR);
			return;
		}

		checkLength(number, cstic, errorObj);
	...
	private void checkLength(final BigDecimal numberToCheck, final CsticData cstic, final Errors errorObj)
	{
		...
		if (actualFraction > maxFractions)
		{
			...
			errorObj.rejectValue("value", TOO_LONG_FRACTION, args, "Too many fractions");
			cstic.setCsticStatus(CsticStatusType.ERROR);
		}
		...
		if (numberOfDecimals > maxDecimals)
		{
			...
			errorObj.rejectValue("value", TOO_MANY_DIGITS, args, "Too many digits");
			cstic.setCsticStatus(CsticStatusType.ERROR);
		}
