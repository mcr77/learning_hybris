...
public class ConfigurationProviderSSCImpl implements ConfigurationProvider
{
	...
	{
		final ConfigModel configModel = configModelFactory.createInstanceOfConfigModel();
		...
		try
		{
			...
			//Root instance
			final IInstanceData rootInstanceData = instanceMap.get(CONFIG).get(0);
			//final IInstanceData instanceData = session.getRootInstance(configId, false);
			...
			final IConfigSession session, final ICsticData[] csticsData) throws IpcCommandException
	{
		...
		{
			final CsticGroupModel csticGroupModel = configModelFactory.createInstanceOfCsticGroupModel();
		...
		for (final ICsticData csticData : csticsData)
		{
			...
			if (groupNames == null || groupNames.length == 0)
			{
				...
				{
					defaultGroup = configModelFactory.createInstanceOfCsticGroupModel();
	...
	{
		final String instanceId = instanceData.getInstId();
		final InstanceModel instanceModel = configModelFactory.createInstanceOfInstanceModel();
	...
	{
		final CsticModel csticModel = configModelFactory.createInstanceOfCsticModel();
	...
	{
		final CsticValueModel csticValueModel = configModelFactory.createInstanceOfCsticValueModel();
	...
	protected final String getValueMapKey(final String instanceId, final String csticName, final String csticValueName)
	{
	...
	protected IConfigSession createSSCSession() throws IpcCommandException
	{
		...
		session.createSession("true", sessionId, SapproductconfigruntimesscConstants.DATA_SOURCE_CRM, false, true, language);
		timer.stop();
