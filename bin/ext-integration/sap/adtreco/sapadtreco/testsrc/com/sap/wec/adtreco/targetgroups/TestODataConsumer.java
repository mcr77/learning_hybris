...
/*		targetGroupReader.setConsumerBuilder(new ODataConsumerBuilder());
		targetGroupReader.setConfiguration(new ADTInitiativesBeCEIImpl());*/

		final List<SAPInitiative> targetGroups = targetGroupReader.getAllInitiatives();
...
/*		targetGroupReader.setConsumerBuilder(new ODataConsumerBuilder());
		...
		final List<SAPInitiative> targetGroups = targetGroupReader.searchInitiatives("abc");
		assertNotNull(targetGroups);
		System.out.println(targetGroups);
...
/*		targetGroupReader.setConsumerBuilder(new ODataConsumerBuilder());
		...
		final List<SAPInitiative> targetGroups = targetGroupReader.searchInitiativesForBP("wec_b2b");
		assertNotNull(targetGroups);
	 ...
	 * 
	 *
	protected ODataConsumer createODataConsumer()
	{
		final ODataClientService testService = new ODataClientService();
