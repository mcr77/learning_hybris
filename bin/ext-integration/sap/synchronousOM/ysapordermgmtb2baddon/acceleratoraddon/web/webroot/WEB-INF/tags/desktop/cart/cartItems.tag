	...
	<table class="cart">
		<thead>
			<tr>
				...
				<th id="header3"><spring:theme code="product.grid.availability"/></th>
				...
				<tr class="cartItem">
					...
					<td headers="header2" class="details">
						<ycommerce:testId code="cart_product_name">
							<div class="itemName"><a href="${productUrl}" style="text-decoration: underline !important;font-weight: bold !important;">${entry.product.name}</a></div>	
						</ycommerce:testId>
						<c:if test="${entry.configurable}">
							<div style="float:right;margin-left:30px;margin-right: 10px;text-decoration:underline;">
								<spring:url	value="${entry.handle}/ ${entry.product.code} /configCartEntry"	var="configUrl"></spring:url>
								<a href="${configUrl}"><spring:theme code="sap.cart.configure"/></a>	
							</div>	
						</c:if>
