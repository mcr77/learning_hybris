...
public class DefaultAbstractOrderEntryPopulator implements Populator<Item, OrderEntryData>
{
	...
	ProductData createProductFromItem(final Item item)
	{
		...
		{
			//item exists in ERP
			productData.setCode(item.getProductGuid().getIdAsString());
		}
		else
		{
			productData.setCode(item.getProductId());
		}
	...
	{
		final List<ScheduleLineData> scheduleLines = new ArrayList();
	...
	/**
	 * Formats product ID taken from backend catalog and prepares it for Hybris. Removes leading zeros.
	 * 
	 * @param input
