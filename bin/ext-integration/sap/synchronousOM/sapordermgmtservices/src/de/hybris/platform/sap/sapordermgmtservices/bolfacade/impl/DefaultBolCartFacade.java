...
public class DefaultBolCartFacade implements BolCartFacade
{
	...
	{
		return (BasketBackend) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BE_CART_ERP);
	}

	OrderBackend getOrderBE()
	{
		return (OrderBackend) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BE_ORDER_ERP);
	}
	...
	{
		Item newItem;
		final String handle = createNewItem(code, quantity, null);
	 ...
	 */
	private String createNewItem(final String code, final long quantity, final ConfigModel configModel)
	{
		...
		final String handle = createNewItem(code, quantity, getCart(), configModel);
		return handle;
	}

	String createNewItem(final String code, final long quantity, final Basket currentCart, final ConfigModel configModel)
	{
		...
		final String handle = newItem.getHandle();

		if (LOG.isDebugEnabled())
		{
			LOG.debug("addToCart for: " + code + ", handle: " + handle);
		}
	...
	public String addConfigurationToCart(final ConfigModel configModel)
	{
		...
		final String handle = createNewItem(productId, 1, configModel);
