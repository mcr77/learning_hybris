...
import de.hybris.platform.sap.sapordermgmtbol.transaction.salesdocument.backend.impl.messagemapping.MessageMappingRulesContainerImpl;
...
public class MessageMappingRulesContainerKeyTest extends TestCase {
	...
	empty = new MessageMappingRulesContainerImpl.Key(null, null, null), //
			emptyIdentical = new MessageMappingRulesContainerImpl.Key(null, null,
					null), //
			sample = new MessageMappingRulesContainerImpl.Key(ID, NUM, SEV), //
			sampleEqual = new MessageMappingRulesContainerImpl.Key(
					cloneString(sample.id), cloneString(sample.number),
					cloneString(sample.severity)), //
			sampleEqual2 = new MessageMappingRulesContainerImpl.Key(
					cloneString(sampleEqual.id),
					cloneString(sampleEqual.number),
			...
			sample2 = new MessageMappingRulesContainerImpl.Key("Z" + ID, "888",
					"A");

	public static MessageMappingRulesContainerImpl.Key //
	equalSet[] = new MessageMappingRulesContainerImpl.Key[] { sample,
			sampleEqual, sampleEqual2 },
			emptySet[] = new MessageMappingRulesContainerImpl.Key[] { empty,
					emptyIdentical },
			differntSet[] = new MessageMappingRulesContainerImpl.Key[] { oDiffCl,
					oDiffNum, oDiffSever };

	@Test
	public void testConstructor() {

		MessageMappingRulesContainerImpl.Key k = new MessageMappingRulesContainerImpl.Key(
				ID, NUM, SEV);
