...
public class OrderImpl extends SalesDocumentImpl implements Order
{
	...
	protected boolean checkOrderQuantities()
	{
		boolean result = true;
		...
		while (items.hasNext())
		{
			...
			// Item is not completely delivered
			if (item.getOverallStatus().isPartiallyProcessed())
			{
				// Quantity was changed
				if (!item.getQuantity().equals(item.getOldQuantity()))
				{
					final BigDecimal qtyOld = item.getOldQuantity();
					final BigDecimal qtyNew = item.getQuantity();
					// Quantity new < Quantity Old
					if (qtyNew.compareTo(qtyOld) < 0)
					{
						// Order Unit of Measure equal Delivered Unit of Measure
						if (item.getUnit().equals(item.getDeliveredQuantityUnit()))
						{
							final BigDecimal qtyDlv = item.getDeliveredQuantity();
							// if Quantity new < Quantity Delivered
							if (qtyNew.compareTo(qtyDlv) < 0)
							{
								final String[] params =
								{ Integer.toString(item.getNumberInt()) };
								final Message msg = new Message(Message.ERROR, "sapsalestransactions.bo.order.item.qtyMinimum", params,
										"");
								addMessage(msg);
								result = false;
							}
						...
						else
						{
							final String[] params =
							{ Integer.toString(item.getNumberInt()) };
							final Message msg = new Message(Message.ERROR, "sapsalestransactions.bo.order.item.qtyChange", params, "");
							addMessage(msg);
							result = false;
						}
					}
