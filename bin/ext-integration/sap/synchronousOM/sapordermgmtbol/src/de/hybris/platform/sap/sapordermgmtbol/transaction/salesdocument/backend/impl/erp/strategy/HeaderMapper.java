...
public class HeaderMapper extends BaseMapper
{
			...
			final Header header)
	{
		...
		readEsHeadComV(esHeadComV, header, readParams.setIpcPriceAttributes, readParams.shippingConditionsAsText, salesDocument,
				backendState, esHeadComR);

		// changeability
		header.setChangeable(backendState.getLoadState().isChangeable());

		readEsHeadComR(esHeadComR, header, readParams, headerPropMap, headerPriceAttribs, itemsPropMap, shop);
		readTtHeadCondV(ttHeadCondV, esHeadComR, header, readParams.headerCondTypeFreight);
			...
			final JCoStructure esHeadComR)
	{
		...
		if (sapLogger.isDebugEnabled())
		{
			...
			debugOutput.append("\n handle         :      " + JCoHelper.getString(esHeadComV, ConstantsR3Lrd.FIELD_HANDLE));
			debugOutput.append("\n external ID    : " + JCoHelper.getString(esHeadComV, "BSTKD"));
			debugOutput.append("\n shipCond       :    " + JCoHelper.getString(esHeadComV, "VSBED"));
			debugOutput.append("\n req. deliv date:    "
					+ ConversionHelper.convertDateToLocalizedString(esHeadComV.getDate("VDATU")));
		...
		final DocumentType docType = DocumentTypeMapping.getDocumentTypeByTransactionGroup(JCoHelper.getString(esHeadComR,
				"TRVOG_R"));
	 ...
	 * @param shop
	 */
	protected void readEsHeadComR(final JCoStructure esHeadComR, final Header header, final GetAllReadParameters readParams,
