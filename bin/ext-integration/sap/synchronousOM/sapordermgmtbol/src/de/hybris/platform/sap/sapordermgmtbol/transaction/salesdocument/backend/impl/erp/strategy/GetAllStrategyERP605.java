...
public class GetAllStrategyERP605 extends GetAllStrategyERP
{
	...
	protected static final Log4JWrapper sapLogger = Log4JWrapper.getInstance(GetAllStrategyERP605.class.getName());
			...
			final ObjectInstances objInstMap, final Map<String, Item> itemMap)
	{
		...
		if (head.isDocumentTypeOrder())
		{
			...
			for (int j = 0; j < numLines; j++)
			{
				...
				final String dlvStatus = JCoHelper.getString(ttItemVstatComV, "LFSTA").length() == 0 ? " " : JCoHelper.getString(
						ttItemVstatComV, "LFSTA");
				final String giStatus = JCoHelper.getString(ttItemVstatComV, "WBSTA").length() == 0 ? " " : JCoHelper.getString(
						ttItemVstatComV, "WBSTA");
				final String rjStatus = JCoHelper.getString(ttItemVstatComV, "ABSTA").length() == 0 ? " " : JCoHelper.getString(
						ttItemVstatComV, "ABSTA");
				final String ordInvoiceStatus = JCoHelper.getString(ttItemVstatComV, "FKSAA").length() == 0 ? " " : JCoHelper
						.getString(ttItemVstatComV, "FKSAA");
				final String dlvInvoiceStatus = JCoHelper.getString(ttItemVstatComV, "FKSTA").length() == 0 ? " " : JCoHelper
						.getString(ttItemVstatComV, "FKSTA");
				final String procStatus = JCoHelper.getString(ttItemVstatComV, "GBSTA").length() == 0 ? " " : JCoHelper.getString(
				...
				shipStatus
						.init(EStatus.getStatusType(dlvIStatus), EStatus.getStatusType(giIStatus), EStatus.getStatusType(rjIStatus));
			...
			final String procStatus = JCoHelper.getString(esHvStatComV, "GBSTK").length() == 0 ? " " : JCoHelper.getString(
					esHvStatComV, "GBSTK");
			final String rjHeaderStatus = JCoHelper.getString(esHvStatComV, "ABSTK").length() == 0 ? " " : JCoHelper.getString(
					esHvStatComV, "ABSTK");
