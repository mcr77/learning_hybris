...
public class PunchOutOrderMessageHeaderProcessingActionTest
{
	...
	public void shouldCreateThePunchOutTransaction()
	{
		...
		assertEquals(NETWORK_ID, senderCredential.getDomain());
		assertEquals(CREDENTIAL_ID, senderCredential.getIdentity().getContent().get(0));
		assertEquals(SHARED_SECRET, ((SharedSecret) senderCredential.getSharedSecretOrDigitalSignatureOrCredentialMac().get(0))
				.getContent().get(0));
