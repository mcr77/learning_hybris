...
public class B2BOrderApprovalBudgetExceededTest extends WorkflowIntegrationTest
{
	@SuppressWarnings("unused")
	private static final Logger LOG = Logger.getLogger(B2BOrderApprovalBudgetExceededTest.class);
	@Resource
	...
	@Test
	public void startApprovalProcessAndCheckBudgetExceededPermission() throws Exception
	{
		boolean isbudgetExceededPermissionApprovalRequired = false;
		B2BPermissionResultModel budgetExccededPermissionModel = null;
		final OrderModel order = createOrder("GC S HH", 999999, OrderStatus.CREATED);
		...
		if (this.waitForProcessAction(b2bApprovalProcessModel.getCode(), "waitProcess", 60000))
		{
			...
			final Collection<B2BPermissionResultModel> permissionResults = order.getPermissionResults();
			for (final B2BPermissionResultModel permissionModel : permissionResults)
			{
				if (permissionModel.getPermissionTypeCode().equals(B2BBudgetExceededPermissionModel._TYPECODE))
				{
					isbudgetExceededPermissionApprovalRequired = true;
					budgetExccededPermissionModel = permissionModel;
				}
			}
			Assert.assertTrue(isbudgetExceededPermissionApprovalRequired);
			Assert.assertNotNull(budgetExccededPermissionModel.getPermission());
