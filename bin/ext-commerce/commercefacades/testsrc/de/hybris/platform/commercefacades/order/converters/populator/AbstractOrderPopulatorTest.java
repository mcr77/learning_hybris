...
public class AbstractOrderPopulatorTest
{
	...
	public void setUp()
	{
		...
		cartPopulator.setDeliveryService(deliveryService);
	...
	@Test
	public void testAddDefaultPaymentInformation()
	{
		...
		cartPopulator.addPaymentInformation(cartModel, cartData);
		Assert.assertEquals(ccPaymentInfoData, cartData.getPaymentInfo());
		Assert.assertTrue(cartData.getPaymentInfo().isDefaultPaymentInfo());
	}

	@Test
	public void testAddNotMatchingDefaultPaymentInformation()
	{
		final CreditCardPaymentInfoModel defaultPaymentInfo = mock(CreditCardPaymentInfoModel.class);
		final CustomerModel customerModel = mock(CustomerModel.class);
		...
		given(customerModel.getDefaultPaymentInfo()).willReturn(defaultPaymentInfo);
		given(cartModel.getUser()).willReturn(customerModel);
		given(cartModel.getPaymentInfo()).willReturn(creditCardPaymentInfoModel);
		given(creditCardPaymentInfoConverter.convert(creditCardPaymentInfoModel)).willReturn(ccPaymentInfoData);
		cartPopulator.addPaymentInformation(cartModel, cartData);
		Assert.assertEquals(ccPaymentInfoData, cartData.getPaymentInfo());
		Assert.assertFalse(cartData.getPaymentInfo().isDefaultPaymentInfo());
	}

	@Test
	public void testAddPaymentInformationWithoutCustomerDefault()
	{
		final CustomerModel customerModel = mock(CustomerModel.class);
		...
		given(customerModel.getDefaultPaymentInfo()).willReturn(null);
		given(cartModel.getUser()).willReturn(customerModel);
		given(cartModel.getPaymentInfo()).willReturn(creditCardPaymentInfoModel);
		given(creditCardPaymentInfoConverter.convert(creditCardPaymentInfoModel)).willReturn(ccPaymentInfoData);
		cartPopulator.addPaymentInformation(cartModel, cartData);
	...
	public void testAddDeliveryMethodZone()
	{
		...
		given(Double.valueOf(priceValue.getValue())).willReturn(Double.valueOf(3.3));
		...
		final ZoneDeliveryModeValueModel zoneDeliveryModeValueModel = mock(ZoneDeliveryModeValueModel.class);
		final CurrencyModel currencyModel = mock(CurrencyModel.class);
		final PriceData priceData = mock(PriceData.class);
		...
		given(
				deliveryService.getDeliveryCostForDeliveryModeAndAbstractOrder(Mockito.any(ZoneDeliveryModeModel.class),
						Mockito.any(CartModel.class))).willReturn(priceValue);
		given(zoneDeliveryModeValueModel.getValue()).willReturn(Double.valueOf(3.3));
		given(currencyModel.getIsocode()).willReturn(ISOCODE);
		given(zoneDeliveryModeValueModel.getCurrency()).willReturn(currencyModel);
		given(priceDataFactory.create(PriceDataType.BUY, BigDecimal.valueOf(3.3), ISOCODE)).willReturn(priceData);
		cartPopulator.addDeliveryMethod(cartModel, cartData);
