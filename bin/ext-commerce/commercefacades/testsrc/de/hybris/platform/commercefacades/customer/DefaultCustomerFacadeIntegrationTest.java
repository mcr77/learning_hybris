...
public class DefaultCustomerFacadeIntegrationTest extends ServicelayerTest
{
	...
	public void testGetCurrentUser()
	{
		...
		AddressData address = customer.getDefaultBillingAddress();
		Assert.assertEquals("Schiller Allee", address.getLine1());
		Assert.assertEquals("8", address.getLine2());
		Assert.assertEquals("28277", address.getPostalCode());
		Assert.assertEquals("Bremen", address.getTown());
		Assert.assertEquals("US", address.getCountry().getIsocode());
		Assert.assertEquals("hybris GmbH", address.getCompanyName());
		address = customer.getDefaultShippingAddress();
		Assert.assertEquals("Zennerstrasse", address.getLine1());
		Assert.assertEquals("45", address.getLine2());
		Assert.assertEquals("28277", address.getPostalCode());
		Assert.assertEquals("Bremen", address.getTown());
		Assert.assertEquals("US", address.getCountry().getIsocode());
		Assert.assertEquals("hybris GmbH", address.getCompanyName());
